	...
	while (!discoveredNode.empty()) //While there are still nodes to be processed
	{
		if (endReached != 1) {
			...
			while (list[connectedNode].edges != NULL) {
				...
				list[nextConnectedNode].edges->connectsToNode;
				if (list[connectedNode].edges->next != NULL) {
					connectedNode = list[nextConnectedNode].edges->next->connectsToNode;
					connectedWeight = list[nextConnectedNode].edges->next->weight;
				}
				else {
					break;
				}
			}
			if (list[connectedNode].edges == NULL) {
				discoveredNode.push_back(connectedNode); //Add node to discovered
				nodeScore.push_back(connectedWeight);	//Calculate weight
				endReached = 1;
			}
			...
			//Processed node
			processedNode.push_back(currentNode);
		}
		else { //Finished
		getXY(currentNode-1, xcord, ycord); //Get cords for current node
		cout << endl << "Checking Node: " << currentNode-1 << " (" << xcord << ", " << ycord << ")" << endl; // You should print as you explore each node

		discoveredNode.erase(discoveredNode.begin() + lowestWeightNodeVectorIndex); //Remove lowest node from discovered as we will begin processing
		nodeScore.erase(nodeScore.begin() + lowestWeightNodeVectorIndex);
		processedNode.push_back(currentNode-1);

		for (int i = 0; i < processedNode.size(); i++) {
			returnList[i] = processedNode[i];
		}
		length = processedNode.size();
		return SUCCESS;
		}
